let sketch1 = p => {
  ...
  p.draw = function() {
    ...
    if (particlesList.length > maximunParticles) {
      let extra = particlesList.slice(maximunParticles, particlesList.length)
        .length;
  ...
  p.mouseClicked = function() {
    ...
    particlesList.push(
      new Particle(p.mouseX, p.mouseY, directionX, directionY)
    );
  };
  ...
  function mouseIsNotOutside(mousePosition) {
    if (
      mousePosition.x != 0 &&
      mousePosition.y != 0 &&
      mousePosition.x != p.windowWidth - 1 &&
      mousePosition.y != p.windowHeight - 1
    ) {
      return true;
    }
  }
  ...
  class Particle {
    ...
    intersects(otherParticle) {
      let distance = p.int(
        p.dist(this.x, this.y, otherParticle.x, otherParticle.y)
      );
